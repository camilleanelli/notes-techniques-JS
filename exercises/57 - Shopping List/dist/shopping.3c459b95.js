...
var list = document.querySelector('.list'); // un tableau pour recuperer les etats ( les données qui refletent l'état de l'application)
...
function handleSubmit(e) {
  ...
  list.dispatchEvent(new CustomEvent('itemsUpdated')); // ici lorsque la fonction est lancée rien ne se passe tant qu'on a pas écouté l'event 'itemsUpdated'
...
function deleteItem(id) {
  ...
  console.log(id); // ici je vais dispatcher le custon event qui est écouter si dessous pour à nouveau afficher la nouvelle version des items:
...
shoppingForm.addEventListener('submit', handleSubmit); // on écoute l'event sur list mais on aurait pu l'appeler sur n'importe quel autre élément

list.addEventListener('itemsUpdated', displayItems); // si j'ai une fonction qui est lancée et qui dispatche cet event, alors la fonction 'displayItems' sera déclenchée
// list.addEventListener('itemsUpdated', e => { console.log(e); })

list.addEventListener('itemsUpdated', mirrorToLocalStorage); // on fait de la délégation d'event: on ecoute un évent sur la liste, mais si l'élément cliqué est button alors on reporte l'action sur celui ci
